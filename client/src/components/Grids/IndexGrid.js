  ...
  return (
    <Grid item key={0} sm={10} md={6} lg={4}>
      <Card className={classes.card}>
        ...
        <CardActions>{showIndexControls(props)}</CardActions>
      </Card>
    </Grid>
  );
}
...
function showIndexStatusText(status) {
  ...
  if (status === "FAILED") {
    return (
      ...
      >
        {" "}
        Indexing failed, please restart
      </p>
    );
  }
}
...
IndexGrid.propTypes = {
  ...
  indexWords: PropTypes.func.isRequired,
...
export default connect(mapStateToProps)(withStyles(styles)(IndexGrid));
